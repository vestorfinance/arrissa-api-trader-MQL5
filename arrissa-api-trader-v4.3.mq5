...
input string S31_input = " ***** Take The Deal Settings *****";
//--- input parameter
input bool EnableTakeTheDealFunction_input = true;
input double TakeProfitMoneyForCycle_input = 500.0;    // Profit target per cycle in account currency
...
//Inputs for take the deal function
bool EnableTakeTheDealFunction = EnableTakeTheDealFunction_input;
double TakeProfitMoneyForCycle = TakeProfitMoneyForCycle_input;    // Profit target per cycle in account currency
...
bool   cycleActive         = false;  // Is a cycle currently in progress?
double cycleStartBalance   = 0.0;    // Balance at the start of the cycle
...
//+------------------------------------------------------------------+
   ...
   if (jjs.Deserialize(message))
     ...
     //Take the deal
      bool EnableTakeTheDealFunction_json               = jjs["EnableTakeTheDealFunction"].ToBool();
      double TakeProfitMoneyForCycle_json               = jjs["TakeProfitMoneyForCycle"].ToDbl();
      ...
      //Take the Deal
      EnableTakeTheDealFunction = EnableTakeTheDealFunction_json;
      TakeProfitMoneyForCycle = TakeProfitMoneyForCycle_json;    // Profit target per cycle in account currency
...
//+------------------------------------------------------------------+
//| Manage trading cycles                                            |
//+------------------------------------------------------------------+
void ManageCycle()
  {
   int totalPositions = PositionsTotal();

   // Start a new cycle when the first trade opens
   if(!cycleActive && totalPositions > 0)
     {
      cycleActive       = true;
      cycleStartBalance = AccountInfoDouble(ACCOUNT_BALANCE);
      PrintFormat("[Cycle] New cycle started. Start Balance: %.2f", cycleStartBalance);
      return;
     }

   if(cycleActive)
     {
      // If all positions have closed, cycle ends
      if(totalPositions == 0)
        {
         cycleActive = false;
         Print("[Cycle] Cycle ended without reaching target.");
         return;
        }

      // Otherwise check if closing now would achieve the target
      double currentEquity   = AccountInfoDouble(ACCOUNT_EQUITY);
      double potentialProfit = currentEquity - cycleStartBalance;

      if(potentialProfit >= TakeProfitMoneyForCycle)
        {
         PrintFormat("[Cycle] Profit target reachable (%.2f). Closing all trades.", potentialProfit);
         CloseAllGlobal();
        }
     }
  }
